...
function logIn(){
		...
		mta.triggerEvent("sign-in", $('#login_username').val(), $('#login_password').val());
	
}

function startRegister(){
	mta.triggerEvent("register", $('#register_username').val(), $('#register_password').val(),$('#register_password2').val(), $('#register_email').val());
}
...
function setUsername(txt){
	$('#login_username').val(txt);
}

function setPassword(txt){
	$('#login_password').val(txt);
}
...
function show(to){
	...
	$("#" + to).show();
	if(to == "login"){
		$("#login_username").removeAttr("readonly");
	}
}
